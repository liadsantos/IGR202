...
bool g_blackLight = false;
bool g_changeLight = false;
bool g_changePositionLight = false;
int aux_changeKey = 0;
...
public:
  ...
  bool allocate(unsigned int width=1024, unsigned int height=768) {
    // creates a framebuffer object for rendering the depth map
    glGenFramebuffers(1, &_depthMapFbo);
    glBindFramebuffer(GL_FRAMEBUFFER, _depthMapFbo);

    // resolution of the depth shadow map
    _depthMapTextureWidth = width;
    _depthMapTextureHeight = height;

    // create a 2D texture that uses the framebuffer's depth buffer
  ...
  // raster image Portable Pixmap (ppm)
...
void printHelp()
{
  std::cout <<
    ...
    "    * C: change light color" << std::endl <<
    "    * B: turn the light on and off" << std::endl <<
    "    * P: change light position" << std::endl <<
...
void keyCallback(GLFWwindow *window, int key, int scancode, int action, int mods) {
  if(action == GLFW_PRESS && key == GLFW_KEY_H) {   // letter h
    printHelp();
  }

  else if(action == GLFW_PRESS && key == GLFW_KEY_S) {  // letter s
    g_scene.saveShadowMapsPpm = true;
  }

  else if(action == GLFW_PRESS && key == GLFW_KEY_T) {  // letter t
    g_appTimerStoppedP = !g_appTimerStoppedP;
    if(!g_appTimerStoppedP)
      g_appTimerLastColckTime = static_cast<float>(glfwGetTime());
  }

  else if(action == GLFW_PRESS && key == GLFW_KEY_F1) { // key f1
    GLint mode[2];
    glGetIntegerv(GL_POLYGON_MODE, mode);
    glPolygonMode(GL_FRONT_AND_BACK, mode[1] == GL_FILL ? GL_LINE : GL_FILL);
  }

  else if (action == GLFW_PRESS && key == GLFW_KEY_B) { // letter b
    aux_changeKey = 1;
    if (g_blackLight == 1)   g_blackLight = false;
    else                     g_blackLight = true;
  }

  else if (action == GLFW_PRESS && key == GLFW_KEY_C) { // letter c
    aux_changeKey = 1;
    if (g_changeLight)   g_changeLight = false;
    else                 g_changeLight = true;
  }

  else if (action == GLFW_PRESS && key == GLFW_KEY_P) { // letter p
    aux_changeKey = 1;
    if (g_changePositionLight)   g_changePositionLight = false;
    else                         g_changePositionLight = true;
  }

  else if(action == GLFW_PRESS && key == GLFW_KEY_ESCAPE) { // key ESC
